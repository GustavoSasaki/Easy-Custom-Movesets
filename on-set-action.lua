...
local function apply_jump_speed(m,stats)
    ...
    if m.action == ACT_JUMP  or m.action ==  ACT_HOLD_JUMP then
        m.vel.y = m.vel.y + 42 * stats.single_jump_strength
         return
     end 
    if m.action == ACT_DOUBLE_JUMP then
        m.vel.y = m.vel.y + 52 * stats.double_jump_strength
       return
    end 
    if m.action == ACT_TRIPLE_JUMP then
        m.vel.y = m.vel.y + 69 * stats.triple_jump_strength
        return
    end 
   if m.action == ACT_BACKFLIP then
       m.vel.y = m.vel.y + 62  * stats.back_flip_strength
       return
   end 

   if m.action == ACT_SIDE_FLIP then
       m.vel.y = m.vel.y + 62  * stats.side_flip_strength
       return
   end 
   if m.action == ACT_LONG_JUMP then
       m.vel.y = m.vel.y + 30  * stats.long_jump_strength
       return
   end 
