...
local function mario_update(m)
    ...
    if (m.action & ACT_AIRBORNE) ~= 0 then
        m.forwardVel = approach_f32(m.forwardVel,0,stats.airborne_deceleration_speed,stats.airborne_deceleration_speed)
        local dragThreshold = m.action and ACT_LONG_JUMP and stats.long_jump_max_velocity or 32;
        if (m.forwardVel > dragThreshold) then
            m.forwardVel =  m.forwardVel - (stats.long_jump_max_velocity/0.35)
        end
        if (m.forwardVel < -16.0) then
            m.forwardVel =  m.forwardVel +  (stats.long_jump_max_velocity * 2/0.35)
        end
    end
end
